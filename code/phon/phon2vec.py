...
def train(dataset_path, output_path):
	...
	model = Word2Vec(
				sentences=ipa_data,
				size=100,  # the length of the output vectors
				window=5,  # that many to the left and that many to the right
				min_count=5,  # ignore tokens occurring less often than that
				sg=1,  # use the skipgram model
				negative=5,  # number of negative samples (per positive one?)
